...
pub fn load_project(fs: &dyn Filesystem, dir: &Path) -> Result<ProjectOnDisk, Error> {
    load_project_inner(fs, dir).map_err(Error)
}

fn load_project_inner(fs: &dyn Filesystem, dir: &Path) -> Result<ProjectOnDisk, ErrorKind> {
    let ProjectDefinition::RootV1(definition) = load_ron(fs, &dir.join("project.ron"))?;
    let tree = load_module_tree(fs, dir)?;
...
mod test {
    ...
    fn loads_the_whole_sample_project() -> Result<(), Error> {
        let project = load_project(&StdFilesystem, &sample_project_dir())?;

        assert_eq!(project.definition.name, "Capstone");
        assert_eq!(project.tree.requirements.len(), 5);
        assert_eq!(project.tree.tests.len(), 4);
    ...
    fn missing_project_ron_is_reported() {
        ...
        let err = load_project(&StdFilesystem, &dir).unwrap_err();
        assert!(matches!(
            err.0,
