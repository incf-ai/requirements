...
/// each keyed by directory name (`attachments`/`templates` are unkeyed —
/// flat lists shared by the whole module, referenced by
/// `requirement.ron`/`test.ron`/`result.ron` files via
/// `AttachmentReferenceKind`/`TemplateReferenceKind` rather than being
/// per-name children). Results are not a direct child of a module tree —
/// each lives nested under its owning requirement
/// (`RequirementOnDisk::results`).
...
/// its requirements/tests/nested-modules tree (each requirement carrying its
/// own nested results).
#[derive(Debug, Clone)]
